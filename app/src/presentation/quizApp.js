...
function gradeColorClass(referenceGrade) {
    if (referenceGrade.startsWith("小"))
        return "grade-elementary";
    if (referenceGrade.startsWith("中"))
        return "grade-middle";
    if (referenceGrade.startsWith("高"))
        return "grade-high";
    return "";
}
/**
 ...
 */
function calcDualProgressPct(mastered, wrong, total) {
    if (total === 0)
        return { masteredPct: 0, inProgressPct: 0 };
...
export class QuizApp {
    ...
    constructor(progressRepo) {
        ...
        this.filter = { subject: "all", category: "all", parentCategory: undefined };
        this.userName = "ゲスト";
        this.questionCount = 10;
        this.notesCanvas = null;
        this.notesStates = new Map();
        this.kanjiCanvasInitialized = false;
        ...
        this.collapsedParentCategories = new Set();
        /** 折りたたまれているトップカテゴリID のセット */
        this.collapsedTopCategories = new Set();
        ...
        this.collapsedGradeGroups = new Set();
        /** 現在選択中のトップカテゴリID（トップカテゴリ選択時のみ設定、単元選択時は null） */
        this.selectedTopCategoryId = null;
        /** フォントサイズレベル（small=デフォルト, medium, large） */
        this.fontSizeLevel = "small";
        /** 総合タブの活動サマリ共有 URL */
        this.shareUrl = "";
        /** 活動サマリで表示する日付（YYYY-MM-DD 形式）: 常に今日の日付 */
        this.selectedActivityDate = QuizApp.currentDateString();
        /** 総合タブから単元を選択した場合の選択情報（null の場合は未選択） */
        this.overallUnitSelected = null;
        /** 総合タブで各教科ごとに表示するおすすめ単元数 */
        this.subjectRecommendedCounts = new Map();
        /** 総合タブの現在アクティブなサマリパネル */
        this.activeOverallPanel = "learned";
        /** 進度タブで選択中の教科ID */
        this.progressSubjectId = SUBJECTS.find((s) => s.id !== "all" && s.id !== "admin" && s.id !== "progress")?.id ?? "english";
        /** 解説コンテンツのロードリクエストカウンタ（レースコンディション防止用） */
        this.guideLoadCounter = 0;
        this.questionListFilter = "all";
        this.quizOrder = "random";
        this.includeMastered = false;
     ...
     */
    loadCategoryViewModeFromStorage() {
    ...
    loadQuizSettings() {
        ...
        // DOM への反映
        const countInput = document.querySelector(`input[name="questionCount"][value="${settings.questionCount}"]`);
        if (countInput)
            countInput.checked = true;
        const orderInput = document.querySelector(`input[name="quizOrder"][value="${settings.quizOrder}"]`);
        if (orderInput)
            orderInput.checked = true;
        const learnedInput = document.querySelector(`input[name="quizLearned"][value="${settings.includeMastered ? "include" : "exclude"}"]`);
        if (learnedInput)
            learnedInput.checked = true;
    }
    /**
     * 現在のクイズ設定を保存する。
     */
    ...
    applyFontSize(level, persist = true) {
    ...
    loadQuestionCountFromDOM() {
        const checked = document.querySelector('input[name="questionCount"]:checked');
        if (checked) {
     ...
     */
    getSelectionLevel() {
        if (this.selectedTopCategoryId !== null)
            return "topCategory";
        if (this.filter.category === "all" && this.filter.parentCategory !== undefined)
    ...
    openUserNameEdit() {
        ...
        const input = document.getElementById("headerUserNameInput");
        if (!nameBtn || !editArea || !input)
            return;
     ...
     */
    setupOverallPanelTabs() {
        document.querySelectorAll(".panel-tab[data-overall-panel]").forEach((tab) => {
            tab.addEventListener("click", () => {
                const panel = tab.dataset.overallPanel;
    ...
    renderAdminContent(categoryList) {
        ...
        const showManageContent = () => {
            ...
            const showTabContent = (tabId) => {
                ...
                const activeTab = tabBar.querySelector(`.admin-manage-tab[data-tab="${tabId}"]`);
                if (activeTab)
                ...
                if (tabId === "import") {
                    ...
                    applyBtn.addEventListener("click", () => {
                        ...
                        void this.showConfirmDialog(`「${sections.find(s => s.fileKey === detectedFileKey)?.title ?? detectedFileKey}」データを選択したファイルの内容で上書きします。よろしいですか？`).then((confirmed) => {
                            if (!confirmed)
                                return;
                            try {
                                if (detectedFileKey === "history") {
                                    this.progressRepo.saveHistory(parsedData);
                                }
                                else if (detectedFileKey === "mastered") {
                                    this.progressRepo.saveMasteredIds(parsedData);
                                }
                                else if (detectedFileKey === "streaks") {
                                    this.progressRepo.saveCorrectStreaks(parsedData);
                                }
                    ...
                    resetBtn.addEventListener("click", () => {
                        void this.showConfirmDialog("すべての学習データを削除します。この操作は元に戻せません。続けますか？").then((confirmed) => {
                            if (confirmed) {
        ...
        const showViewContent = () => {
            ...
            const showDataTab = (index) => {
                tabBar.querySelectorAll(".admin-data-tab").forEach((t) => t.classList.remove("active"));
                const activeTab = tabBar.querySelectorAll(".admin-data-tab")[index];
                if (activeTab)
                    activeTab.classList.add("active");
    ...
    renderCategoryListByCategory() {
        ...
        const parentCatIdsInTopGroups = new Set();
        for (const [topCatId, topCatName] of Object.entries(topCategories)) {
            ...
            topGroupHeader.addEventListener("keydown", (e) => {
        ...
        for (const [parentCatId, parentCatName] of Object.entries(allParentCategories)) {
            if (parentCatIdsInTopGroups.has(parentCatId))
                continue;
            const cats = categoriesByParent.get(parentCatId) ?? {};
            if (Object.keys(cats).length === 0)
                continue;
    ...
    renderCategoryListByGrade() {
        ...
        for (const grade of grades) {
            // 学年フィルターで絞る
            if (this.selectedGradeFilter && !grade.startsWith(this.selectedGradeFilter))
                continue;
    ...
    renderCategoryViewControls() {
        ...
        // 各学年プレフィックスボタン
        const labelMap = { "小学": "小学", "中学": "中学", "高校": "高校" };
     ...
     */
    gradeFilterMatches(subject, categoryId) {
        if (!this.selectedGradeFilter)
            return true;
     ...
     */
    buildParentCategoryGroup(subject, parentCatId, parentCatName, cats, topCatId) {
        const groupDiv = document.createElement("div");
        groupDiv.className = "category-group";
        groupDiv.dataset.parentCategory = parentCatId;
        if (topCatId)
            groupDiv.dataset.topCategory = topCatId;
        const groupHeader = document.createElement("div");
    ...
    renderAllSubjectList() {
        ...
        for (const subject of nonAllSubjects) {
            ...
            for (const recommended of recommendedList) {
                ...
                const key = `${topCatId}::${parentCatId}`;
                if (!groupMap.has(key)) {
            ...
            groupOrder.sort((a, b) => {
                ...
                if (aHasUnlearned && !bHasUnlearned)
                    return -1;
                if (!aHasUnlearned && bHasUnlearned)
                    return 1;
                return 0;
     ...
     */
    filterRecordsBySelectedDate(records) {
        const dateToCheck = this.parseActivityDate().toDateString();
        return records.filter((r) => new Date(r.date).toDateString() === dateToCheck &&
            r.mode !== "manual" &&
     ...
     * 総合タブ専用のサマリパネルを描画する。
     */
    renderOverallSummaryPanel(allRecords) {
    ...
    updateActivityDateDisplay() {
        ...
        const symbols = "🏆".repeat(Math.min(masteredCount, 5)) +
    ...
    showOverallPanel(tab) {
        ...
        document.querySelectorAll(".panel-tab[data-overall-panel]").forEach((t) => {
     ...
     */
    updateShareSummaryText(records) {
    ...
    buildShareSummaryText(records) {
        ...
        const subjectIconMap = Object.fromEntries(SUBJECTS.filter((s) => s.id !== "all" && s.id !== "admin").map((s) => [s.id, s.icon]));
        // 教科＋単元ごとに集計
        ...
        for (const [key, data] of byUnit.entries()) {
            ...
            const pathParts = [data.subjectName];
            const shouldIncludeTopCategory = topCat !== undefined && topCat.name !== parentCat?.name;
            if (shouldIncludeTopCategory && topCat)
                pathParts.push(topCat.name);
            if (parentCat)
                pathParts.push(parentCat.name);
     ...
     */
    updateShareUrlOpenBtn() {
        const btn = document.getElementById("openShareUrlBtn");
        if (btn) {
            if (this.shareUrl) {
    ...
    createCategoryItem(subject, categoryId, categoryName, parentCatId, topCatId) {
        const item = document.createElement("div");
        ...
        if (this.categoryViewMode === "grade") {
            ...
            const nameParts = [];
            if (topInfo)
                nameParts.push(topInfo.name);
            if (parentInfo)
                nameParts.push(parentInfo.name);
    ...
    updateCategoryListActive() {
        ...
        categoryList.querySelectorAll(".category-group-header[data-parent-category]").forEach((header) => {
            const isActive = selLevel === "parentCategory" &&
                header.dataset.parentCategory === this.filter.parentCategory;
            header.classList.toggle("active", isActive);
        });
        // トップカテゴリヘッダーのアクティブ状態を更新
        categoryList.querySelectorAll(".category-top-group-header[data-top-category]").forEach((header) => {
            const isActive = selLevel === "topCategory" &&
    ...
    autoSelectPanelTab(allRecords) {
        ...
        if (this.overallUnitSelected !== null) {
            const hasHistory = allRecords.some((r) => r.subject === this.overallUnitSelected.subject &&
        ...
        if (selLevel === "topCategory" || selLevel === "parentCategory") {
            ...
            const hasHistory = allRecords.some((r) => r.subject === this.filter.subject && r.category === this.filter.category);
            this.activePanelTab = hasHistory ? "quiz" : "guide";
        }
        else {
            this.activePanelTab = "quiz";
    ...
    updateSelectedUnitInfo() {
        ...
        if (this.overallUnitSelected !== null) {
            ...
            const catParts = [];
            if (topInfo)
                catParts.push(topInfo.name);
            if (parentInfo)
                catParts.push(parentInfo.name);
            const grade = this.useCase.getCategoryReferenceGrade(subject, categoryId);
            const example = this.useCase.getCategoryExample(subject, categoryId);
            const description = this.useCase.getCategoryDescription(subject, categoryId);
            // 行1: タイトル（左） / 説明（右）
     ...
     */
    showParentCategoryGuide(guideUrl) {
        this.activePanelTab = "guide";
        this.isPanelTabUserSelected = true;
     ...
     */
    updateGuidePanelContentByIds(frameId, noContentId) {
        ...
        if (this.overallUnitSelected !== null) {
            ...
            guideUrl = this.useCase.getCategoryGuideUrl(this.overallUnitSelected.subject, this.overallUnitSelected.categoryId);
        }
        else {
            const selLevel = this.getSelectionLevel();
            if (selLevel === "unit") {
     ...
     */
    async loadGuideContent(container, guideUrl, noContent) {
        // 既に同じ URL を表示中なら再ロードしない
        if (container.dataset.loadedUrl === guideUrl)
            return;
        ...
        try {
            ...
            if (parsed.origin !== window.location.origin) {
                if (token !== this.guideLoadCounter)
                    return;
        ...
        try {
            ...
            bodyClone.querySelectorAll("header.site-header, footer.site-footer, footer, header.post-header, style, .site-nav, .site-title, a.site-title").forEach((el) => el.remove());
            ...
            let guideContent = container.querySelector(".guide-content");
            if (!guideContent) {
     ...
     */
    renderHistoryList(filter, allRecords) {
        const historyList = document.getElementById("historyList");
        if (!historyList)
            return;
        const records = (allRecords ?? this.useCase.getHistory()).filter((r) => (filter.subject === "all" || r.subject === filter.subject) &&
     ...
     */
    buildHistoryItem(record, showSubjectPrefix = false) {
        const item = document.createElement("div");
    ...
    renderQuestionList() {
        ...
        if (countEl)
            countEl.textContent = `${questions.length}問`;
        bodyEl.innerHTML = "";
     ...
     */
    buildQuestionListItem(question) {
        const item = document.createElement("div");
    ...
    setupEventListeners() {
        ...
        // 問題一覧フィルターボタン
        const filterBtns = [
            { id: "questionListFilterAll", value: "all" },
            { id: "questionListFilterUnlearned", value: "unlearned" },
            { id: "questionListFilterLearned", value: "learned" },
        ];
        ...
        headerUserNameInput?.addEventListener("keydown", (e) => {
            if (e.key === "Enter") {
                e.preventDefault();
        ...
        headerUserNameInput?.addEventListener("blur", (e) => {
        ...
        // 並び順選択の変更を監視
        const orderInputs = document.querySelectorAll('input[name="quizOrder"]');
        orderInputs.forEach((input) => {
            input.addEventListener("change", (e) => {
                const target = e.target;
                if (target.checked) {
                    this.quizOrder = target.value;
                    this.saveQuizSettings();
                }
            });
        });
        // 学習済み含む/含まない選択の変更を監視
        const learnedInputs = document.querySelectorAll('input[name="quizLearned"]');
        ...
        document.addEventListener("mousedown", (e) => {
            ...
            if (inline && !inline.contains(e.target)) {
                this.closeShareUrlEdit();
            }
        });
    ...
    navigateToAdmin() {
        ...
        const adminTab = tabsContainer.querySelector('.subject-tab[data-subject="admin"]');
        if (adminTab) {
    ...
    updateSubjectStats() {
        ...
        const addStat = (key, isInProgress, isMastered) => {
        ...
        const formatCategoryStats = (stat) => {
            if (stat.total === 0)
                return "";
            // 常に進捗数値を表示する
        ...
        document.querySelectorAll(".category-item[data-subject]").forEach((item) => {
            ...
            // 進捗バーと進捗数値を更新
            const progressFill = el.querySelector(".category-progress-fill");
            const progressFillInProgress = el.querySelector(".category-progress-fill-inprogress");
            if (progressFill) {
     ...
     */
    toggleParentCategory(parentCatId) {
        if (this.collapsedParentCategories.has(parentCatId)) {
     ...
     */
    expandParentCategory(parentCatId) {
        if (!this.collapsedParentCategories.has(parentCatId))
            return;
        this.collapsedParentCategories.delete(parentCatId);
        this.applyParentCategoryCollapsedState(parentCatId);
     ...
     */
    applyParentCategoryCollapsedState(parentCatId) {
        const categoryList = document.getElementById("categoryList");
        if (!categoryList)
            return;
        const isCollapsed = this.collapsedParentCategories.has(parentCatId);
        const groupDiv = categoryList.querySelector(`.category-group[data-parent-category="${parentCatId}"]`);
        if (groupDiv) {
            groupDiv.classList.toggle("collapsed", isCollapsed);
            const groupHeader = groupDiv.querySelector(".category-group-header");
     ...
     */
    applyTopCategoryCollapsedState(topCatId) {
        const categoryList = document.getElementById("categoryList");
        if (!categoryList)
            return;
        const isCollapsed = this.collapsedTopCategories.has(topCatId);
        const topGroupDiv = categoryList.querySelector(`.category-top-group[data-top-category="${topCatId}"]`);
        if (topGroupDiv) {
            topGroupDiv.classList.toggle("collapsed", isCollapsed);
            const topGroupHeader = topGroupDiv.querySelector(".category-top-group-header");
    ...
    isCurrentCategoryLearned() {
        const effectiveFilter = this.getEffectiveFilter();
        if (effectiveFilter.category === "all")
            return false;
    ...
    updateQuizPanelVisibility() {
        ...
        if (this.filter.subject === "admin") {
            ...
            const adminStatusFilter = document.querySelector(".category-status-filter");
            if (adminStatusFilter)
                adminStatusFilter.classList.add("hidden");
            // 管理タブでは日付ナビを非表示にする
        ...
        if (this.filter.subject === "progress") {
            ...
            const progressStatusFilter = document.querySelector(".category-status-filter");
            if (progressStatusFilter)
                progressStatusFilter.classList.add("hidden");
            // 進度タブでは日付ナビを非表示にする
            document.getElementById("overallDateNav")?.classList.add("hidden");
        ...
        const statusFilterEl = document.querySelector(".category-status-filter");
        if (statusFilterEl)
            statusFilterEl.classList.toggle("hidden", isAll || this.filter.subject === "admin");
    ...
    async startQuiz(mode) {
        ...
        const effectiveMode = (mode === "random" && this.quizOrder === "straight") ? "practice" : mode;
        try {
    ...
    renderQuestion() {
        ...
        const topicParts = [];
        if (question.topCategoryName)
            topicParts.push(question.topCategoryName);
        if (question.parentCategoryName)
            topicParts.push(question.parentCategoryName);
    ...
    renderChoices(question, session) {
     ...
     * 問題遷移のたびに呼ばれる。
     */
    updateNotesAreaForQuestion(question, isAnswered) {
        ...
        if (showKanji) {
            ...
            if (kanjiInputBody)
                kanjiInputBody.classList.remove("hidden");
            if (kanjiToggleBtn)
                this.applyKanjiToggleBtnState(kanjiToggleBtn, true);
        }
     ...
     */
    isKanjiCanvasAvailable() {
        return typeof globalThis.KanjiCanvas !== "undefined";
    }
    /**
     ...
     */
    applyKanjiToggleBtnState(btn, expanded) {
     ...
     */
    loadRefPatterns() {
        if (this.refPatternsLoadPromise)
     ...
     */
    updateKanjiCandidates() {
        const candidateList = document.getElementById("kanjiCandidateList");
        if (!candidateList || !this.isKanjiCanvasAvailable())
            return;
    ...
    showResultScreen(results) {
        ...
        if (resultUnitName) {
            ...
            if (firstResult) {
                ...
                const parts = [];
                if (q.subjectName)
                    parts.push(q.subjectName);
                if (q.topCategoryName)
                    parts.push(q.topCategoryName);
                if (q.parentCategoryName)
                    parts.push(q.parentCategoryName);
                if (q.categoryName)
                    parts.push(q.categoryName);
                if (parts.length > 0) {
        ...
        if (resultMessage) {
            ...
            if (percentage === 100)
                ...
                message = "😊 もう少し！次はきっとできる！";
            else
                message = "💪 がんばれ！次は必ず正解できます！";
     ...
     */
    async checkAllMasteredAndCongratulate() {
        const effectiveFilter = this.getEffectiveFilter();
        const filteredQuestions = this.useCase.getFilteredQuestions(effectiveFilter);
     ...
     */
    showConfirmDialog(message, alertOnly = false) {
     ...
     */
    selectFirstUnlearnedCategory() {
        // URLパラメータまたはフラグメントで特定カテゴリが指定されている場合はディープリンク挙動を維持する
        if (this.getURLParams().has("category"))
            return;
        const categoryList = document.getElementById("categoryList");
        if (!categoryList)
            return;
        const firstUnlearned = categoryList.querySelector(".category-item:not(.learned)");
        if (!firstUnlearned)
            return;
    ...
    // ─── ユーティリティ ────────────────────────────────────────────────────────
    updateUserNameDisplay(elementId) {
