...
export class QuizUseCase {
    ...
    getParentCategoriesForSubject(subject) {
    ...
    getCategoriesForParent(subject, parentCategory) {
        const categories = {};
        for (const q of this.allQuestions) {
    ...
    getTopCategoriesForSubject(subject) {
        const topCategories = {};
        for (const q of this.allQuestions) {
    ...
    getParentCategoriesForTop(subject, topCategory) {
        const parentCategories = {};
        for (const q of this.allQuestions) {
    ...
    startSession(mode, filter, count = 10) {
        const filtered = this.getFilteredQuestions(filter);
        if (mode === "random") {
    ...
    startSessionWithAllQuestions(mode, filter, count = 10) {
        const filtered = this.getFilteredQuestions(filter);
    ...
    getCorrectStreak(questionId) {
        return this.correctStreaks[questionId] ?? 0;
    }
    /**
     * クイズ結果を履歴に追加して保存する。
     */
    addHistoryRecord(results, filter, mode) {
        if (results.length === 0)
            return;
     ...
     */
    markCategoryAsLearned(filter) {
        if (filter.category === "all" || filter.subject === "all")
            return;
     ...
     * category が "all" の場合は一括操作を防ぐために何もしない。
     */
    unmarkCategoryAsLearned(filter) {
        if (filter.category === "all" || filter.subject === "all")
            return;
        const questions = this.getFilteredQuestions(filter);
        if (questions.length === 0)
     ...
     */
    getCategoryGuideUrl(subject, category) {
     ...
     */
    getParentCategoryGuideUrl(subject, parentCategory) {
     ...
     */
    getTopCategoryGuideUrl(subject, topCategory) {
     ...
     */
    getParentCategoryForUnit(subject, categoryId) {
     ...
     */
    getTopCategoryForUnit(subject, categoryId) {
     ...
     */
    getCategoryExample(subject, category) {
     ...
     */
    getCategoryReferenceGrade(subject, category) {
     ...
     */
    getRecommendedCategoryForSubject(subject) {
     ...
     * カテゴリが存在しない場合は空配列を返す。
     */
    getRecommendedCategoriesForSubject(subject, count) {
        ...
        if (entries.length === 0)
            return [];
        const wrongSet = new Set(this.wrongIds);
        const wrongCountsByCategory = new Map();
        for (const question of this.allQuestions) {
            if (question.subject !== subject || !wrongSet.has(question.id))
                continue;
     ...
     */
    getCategoryProgressPct(subject, categoryId) {
        let total = 0;
        let mastered = 0;
        let wrong = 0;
        const wrongSet = new Set(this.wrongIds);
        for (const q of this.allQuestions) {
            if (q.subject !== subject || q.category !== categoryId)
                continue;
            total++;
     ...
     */
    getMasteredCountForCategory(subject, categoryId) {
        let total = 0;
        let mastered = 0;
        for (const q of this.allQuestions) {
            if (q.subject !== subject || q.category !== categoryId)
     ...
     * 学習記録がない場合は null を返す。
     */
    getLastStudyDateForSubject(subject) {
        const history = this.progressRepo.loadHistory();
     ...
     */
    getCategoriesForGrade(subject, grade) {
        const categories = {};
        for (const q of this.allQuestions) {
     ...
     */
    getCategoriesWithoutGrade(subject) {
        const categories = {};
        for (const q of this.allQuestions) {
