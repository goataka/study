...
describe("expandQuestions — 漢字（読み・書き）の10字分割仕様", () => {
  it("漢字の読み問題は10字ごとにカテゴリを分割し、タイトルに範囲を付与する", () => {
    const qf: QuestionFile = {
      subject: "japanese",
      subjectName: "国語",
      category: "kanji-grade1",
      ...
      questions: Array.from({ length: 12 }, (_, i) => ({
        id: `00000000-0000-5000-8000-${String(i + 1).padStart(12, "0")}`,
        question: `問題${i + 1}`,
        choices: [`答え${i + 1}`],
        correct: 0,
        explanation: `解説${i + 1}`,
    ...
    expect(questions[0]?.category).toBe("kanji-grade1");
    expect(questions[9]?.category).toBe("kanji-grade1");
    expect(questions[10]?.category).toBe("kanji-grade1-2");
    expect(questions[0]?.categoryName).toBe("📖 読み（1-10字）");
    expect(questions[10]?.categoryName).toBe("📖 読み（11-12字）");
  });
  ...
  it("漢字以外のカテゴリは分割しない", () => {
    const qf: QuestionFile = {
      ...
      questionType: "text-input",
      questions: Array.from({ length: 12 }, (_, i) => ({
        id: `10000000-0000-5000-8000-${String(i + 1).padStart(12, "0")}`,
        question: `問題${i + 1}`,
        choices: [`答え${i + 1}`],
        correct: 0,
        explanation: `解説${i + 1}`,
    ...
    for (const q of questions) {
      expect(q.category).toBe("addition");
      expect(q.categoryName).toBe("➕ たし算");
    }
  });
});
