...
describe("validateQuestionFile — 問題ファイル検証仕様", () => {
  ...
  it("parentCategoryGuideUrl が文字列でない場合は拒否する", () => {
    expect(() =>
      validateQuestionFile({ ...validQF, parentCategoryGuideUrl: 123 })
...
describe("expandQuestions — 問題展開仕様", () => {
  const qf: QuestionFile = {
    ...
    categoryName: "足し算",
    questions: [
      { id: "d013bc91-9c9a-5d3e-9c3e-54df258a661c", question: "1+1=?", choices: ["1", "2", "3", "4"], correct: 1, explanation: "1+1=2" },
      { id: "a713882e-64f5-50e6-acd6-e9f68925ae3a", question: "2+2=?", choices: ["2", "3", "4", "5"], correct: 2, explanation: "2+2=4" },
    ],
  };
...
describe("expandQuestions — questionType 継承仕様", () => {
  it("ファイルレベルの questionType が各問題に継承される", () => {
    const qf = {
      ...
      questions: [
        { id: "7e09165c-058d-505b-a43b-802be91b90b5", question: "テスト", choices: ["こたえ"], correct: 0, explanation: "解説" },
      ],
    };
    const questions = expandQuestions(qf);
  ...
  it("questionType が未指定の場合 multiple-choice がデフォルトになる", () => {
    const qf = {
      ...
      categoryName: "テスト",
      questions: [
        { id: "2bddd8e7-8a2e-5a3c-b9c4-9b5f35f254e1", question: "テスト", choices: ["ア", "イ", "ウ", "エ"], correct: 0, explanation: "解説" },
      ],
    };
    const questions = expandQuestions(qf);
    expect(questions[0]!.questionType).toBe("multiple-choice");
