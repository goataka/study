...
デグレード検知を強化するため、E2E feature は用途別に分割しています。

| フォルダ | 目的 |
|------|------|
| `app/e2e/features/usecases/` | 利用者の通しフロー（例: スタート→学習→結果）を保証する |
| `app/e2e/features/components/` | ヘッダー・タブ・管理画面などパーツ単位の挙動を保証する |

各シナリオは検証ステップで VR スナップショットを取得し、`app/e2e/snapshots/` に保存された画像と照合します。これにより、シナリオと画像の対応を明確に保ったまま、広い範囲のUI退行を検出できます。
