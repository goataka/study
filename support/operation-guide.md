...
| 候補ボタン | 書いた文字の認識候補を表示する。クリックすると解答欄に入力される（英語問題ではアルファベット候補を優先して表示し、認識できない場合は全候補をフォールバック表示する） |
...
| サマリーカード | 学習済み単元数や達成率を教科ごとに表示する |
| カテゴリ別進度 | 各カテゴリの達成状況を一覧で確認できる |
| 学年別進度 | 学年ごとの達成状況を一覧で確認できる |
...
| タブ | 説明 |
|------|------|
| 設定 | ユーザー名・表示設定など保存された設定を確認する |
| 履歴 | クイズ履歴データをJSON形式で確認する |
| 学習済み問題 | 学習済みとして保存されている問題IDを確認する |
| 連続正解 | 連続正解数の保存データを確認する |

### 仕様メニュー（📋 仕様）

| パーツ | 説明 |
|--------|------|
| データ構造一覧 | 学習データのキーや構造を説明する |
| 運用メモ | インポート・エクスポート時の注意点を確認できる |
