...
| 次のおすすめ | 最初に取り組むおすすめ単元名（教科絵文字付き）を星の下に表示する（単元名は太字） |
| 開始するボタン | おすすめ先頭単元を開く（未学習は `📖 解説`、学習済みは `✅ 確認`） |
| 今日やった単元 | 開始するボタンの下に当日学習した単元を表示する（単元リストのみスクロール） |

#### 今日の単元リスト

| パーツ | 説明 |
|--------|------|
| タイトル右の ℹ️ | 「当日学習記録のある単元をおすすめ順で表示」の条件を示す |
| 目標数コントロール | おすすめ表示の目標数（2/3/5/8/13）を切り替える |
| 単元カード | 教科絵文字・単元名・進捗バーを表示。クリックで単元を選択する |
...
| 候補ボタン | 書いた文字の認識候補を表示する。クリックすると解答欄に入力される（英語問題では正解文字列も常に候補として表示） |
