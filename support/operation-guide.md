...
| 次の単元 | 最初に取り組むおすすめ単元名（教科名・単元名とも太字）を星の下に表示する |
...
| パーツ | 説明 |
|--------|------|
| タイトル右の ℹ️ | 抽出条件を表示する（未学習優先 / 学習済7日後・復習済14日後の復習対象 / 修了済除外 / 国語→数学→英語優先） |
...
| 単元アイテム | クリックすると単元を選択し、右パネルに詳細が表示される。単元名の横に `🎖️` 学習済 / `🏆` 復習済 / `👑` 修了済 を表示する |
...
| 候補ボタン | 書いた文字の認識候補を表示する。クリックすると解答欄に入力される（英語問題では認識されたアルファベット候補のみ表示し、正解は自動表示しない） |
