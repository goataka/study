...
機能追加・変更の完了報告前に、以下のチェックリストをすべて確認すること。1つでも「いいえ」がある場合はコミットしてはならない：

- [ ] マニュアル（`support/operation-guide.md` または `support/startup-guide.md`）に、追加・変更した機能の説明が記載されているか？
- [ ] 追加・変更した機能に対応するE2Eシナリオ（`e2e/features/*.feature`）が存在するか？
- [ ] E2Eシナリオに対応するステップ定義（`e2e/steps/*.ts`）が存在するか？
- [ ] `pnpm run test:e2e` がすべてパスするか？
- [ ] 実装のみが先行して、マニュアルやE2Eが後回しになっていないか？（手順の順序を遵守したか？）

### 手順違反の防止策

- **プランニング時に明示する**: 作業計画（report_progress のチェックリスト）には必ず「マニュアル更新」「E2Eシナリオ作成」「ステップ定義作成」「E2Eテスト実行」の項目を含めること
- **実装コードのみのコミットを禁止する**: UI 機能の変更を含むコミットには、対応するマニュアル変更またはE2Eシナリオ変更が同時に含まれていなければならない
- **順序の明示**: 作業ログの最初に「手順1: マニュアル更新 → 手順2: E2Eシナリオ → 手順3: 実装 → 手順4: ステップ定義 → 手順5: E2Eテスト実行」と記載し、各ステップの完了を明記しながら進めること
