Feature: 学習アプリ
  ...
  Scenario: 親カテゴリグループを折りたたみ・展開できる
    When I click the "英語" tab
    And I click the "tenses" category group header
    Then the "tenses" category group should be collapsed
    When I click the "tenses" category group header again
    Then the "tenses" category group should be expanded
